...
`CodeJarPro` 是在广受欢迎的 [antonmedv/codejar](https://github.com/antonmedv/codejar) 基础上，由 [木炭 (WOODCOAL)](https://github.com/woodcoal) 进行深度重构和功能增强的开源项目。它不仅完整继承了 `codejar` 的轻量与高效（gzip 后仅几 KB），更引入了强大的**模块化插件系统**，并内置了**行号**与**代码标记**等核心插件，让小巧的编辑器也能拥有媲美 IDE 的专业能力与扩展性。
...
-   **内置核心插件**: 开箱即用，提供了开发者最常用的**行号 (`LineNumbers`)** 和**代码标记 (`InsertMark`)** 插件，无需额外配置。
...
-   **`highlight?: (editor: HTMLElement) => void`** : 可选。一个用于语法高亮的函数。当编辑器内容更新时，此函数会被调用，并传入编辑器元素作为参数，你可以在函数内部对 `editor.innerHTML` 进行处理以实现语法高亮。
...
-   `disableDebug: boolean` (默认: `true`): 是否开启调试，开启后将在控制台打印运行日志。
...
-   **`cjp.addPlugin(name, plugin, config?)`** : 注册一个插件。返回插件实例，你可以通过它调用插件的专属 API。
-   **`cjp.removePlugin(name)`** : 根据名称安全地卸载一个插件。
...
你可以轻松创建自己的插件来扩展编辑器功能。一个插件本质上是一个返回特定接口对象的函数。
